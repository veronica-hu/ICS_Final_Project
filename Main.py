...
class MainPanel:
    ...
    def show(self):
        ...
        # 三个frame
        self.function_frame = Frame(main_window, bg='#333333')
        self.send_frame = Frame(main_window, bg='#D3D3D3')
        ...
        self.send_button = Button(main_window, text='send', 
                                command=self.send)
        self.send_button.place(x=630, y=350, height=75, width=70)

        # 文字区
        self.show_text = Text(self.text_frame, font=('Calibri', 16), bg="#DCDCDC",
            highlightcolor='#DCDCDC', highlightthickness=1)
        self.show_text.place(x=0, y =0, height=350, width=550)
        self.show_text.insert(INSERT, 'Welcome to ICS chatroom\n')
        self.show_text.insert(END, 'Connect a peer and chat away!\n\n')
        self.show_text.config(state=DISABLED)  #显示文本区域的文字不可编辑（注意放在最后

        self.send_text = Text(self.send_frame, font=('Calibri', 16), bg='#D3D3D3', 
            highlightcolor='#D3D3D3', highlightthickness=1)
        self.send_text.place(x=0, y=0, height=150, width=480)
        
        #左侧显示who的文字区
        self.peer_text = Text(self.function_frame, font=('Calibri', 16), bg='white', 
        ...
        self.send_frame.place(x=150,y=350, height=150, width=550)
        self.text_frame.place(x=150,y=0, height=350,width=550)
...
#======================End of GUI Setting, Start of commands======================================================
    def connect_peer(self):
        ...
        ok_button = Button(self.connect_window, text='ok', command= self.connect)
        ## 现在okbutton点击后没有用
        ok_button.pack()
